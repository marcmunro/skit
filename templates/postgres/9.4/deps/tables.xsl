...
<xsl:stylesheet
  ...
  <xsl:template match="replica_ident">
    <xsl:param name="parent_core" select="'NOT SUPPLIED'"/>
  ...
  <xsl:template match="replica_ident" mode="dependencies">
    <xsl:param name="parent_core" select="'NOT SUPPLIED'"/>
    ...
    <xsl:if test="@replica_index">
      <xsl:variable name="replica_index" select="@replica_index"/>
      <xsl:choose>
	<xsl:when test="../constraint[@name=$replica_index]">
