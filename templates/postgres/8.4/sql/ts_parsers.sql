select p.prsname as name,
 ...
 inner join pg_proc s
    on s.oid = p.prsstart
 inner join pg_namespace ns
    on ns.oid = s.pronamespace
 inner join pg_proc e
    on e.oid = p.prsend
 inner join pg_namespace ne
    on ne.oid = e.pronamespace
 inner join pg_proc t
    on t.oid = p.prstoken
 inner join pg_namespace nt
    on nt.oid = t.pronamespace
 inner join pg_proc l
    on l.oid = p.prslextype
 inner join pg_namespace nl
    on nl.oid = l.pronamespace
  left outer join pg_proc h
    on h.oid = p.prsheadline
  left outer join pg_namespace nh
